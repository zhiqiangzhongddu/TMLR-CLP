...
class RandomPartitionGraph(InMemoryDataset):
    def __init__(self, root, name, num_train_per_class=20, num_val_per_class=50, num_test_per_class=100,
        ...
        # syn-{n_classes}-{nodes_per_class}-{p_in_ratio}-{avg_degree_ratio}
        #   e.g., syn-10-500-0.9-0.025
        self.name = name
    ...
    def processed_file_names(self):
        return ['syn-data-{}.pt'.format(self.name)]
...
def make_feat(path, name, y):
    feat_path = os.path.join(path, "{}.ogb-products-allx.npy".format(name))
    if os.path.exists(feat_path):
        print('loading OGB-Products feature...')
        feat_x = torch.FloatTensor(np.load(feat_path))
    else:
        print('preprocessing OGB-Products feature...')
        ogb_data = PygNodePropPredDataset(
            name='ogbn-products', root='../data/OGB'
        )[0]
        feat_x = torch.zeros((y.size(0), ogb_data.num_features))
        for item in y.unique():
            all_feat = ogb_data.x[ogb_data.y.view(-1) == item]
            mask = torch.where(y==item)[0]
            feat_x[mask] = all_feat[random.sample(range(all_feat.size(0)), mask.size(0))]
        np.save(feat_path, feat_x.numpy())
